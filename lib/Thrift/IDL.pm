...
sub parse_thrift_file {
    ...
    my ($children, $headers) = $class->_parse_input_fn($input_fn, $debug, {});
...
sub _parse_input_fn {
    my ($class, $input_fn, $debug, $state) = @_;

    if ($state->{_parse_input_fn_processed}{$input_fn}++) {
    ...
    if ($data !~ m{^\s*$}s) {
        print "Parsing failed to consume all of the file $input_fn; stopped at:\n";
    ...
    foreach my $child (@$parsed) {
        ...
        if ($child->isa('Thrift::IDL::Comment')) {
            ...
            if ($child->isa('Thrift::IDL::Include')) {
                ...
                my ($include_parsed, $include_headers) = $class->_parse_input_fn($include_fn, $debug, $state);
